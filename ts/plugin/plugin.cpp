...
extern HMODULE xaudioLibrary;

static std::string pluginId{ 0 };

static struct TS3Functions ts3Functions;


static std::map<uint64, std::map<anyID, AudioVoice>> serverClientVoiceMap;
static std::unique_ptr<AudioEngine> audioEngine{ nullptr };
...
void LogWindowsError(const char* message, DWORD error, uint64 serverConnectionID = 0)
{
	char buffer[1024];
	FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, 0, error, 0, buffer, sizeof(buffer) - 1, 0);
	char result[1024];
	sprintf_s(result, "%s (%s)", message, buffer);
	ts3Functions.logMessage(buffer, LogLevel_ERROR, "Plugin", serverConnectionID);
}


template<typename Func>
bool Win32ExceptionBoundary(Func&& f, uint64 serverConnectionID = 0)
{
	try
	{
		f();
	...
	catch (Win32Exception& e)
	{
		...
		ts3Functions.logMessage(e.what(), LogLevel_ERROR, "Plugin", serverConnectionID);
	}
...
bool LoadXAudioLibrary(const char* pluginPath, const char* relativeDllPath)
{
	if (!xaudioLibrary) {
		char xaudioLibPath[PATH_BUFSIZE];
		strcpy_s(xaudioLibPath, pluginPath);
		strcat_s(xaudioLibPath, relativeDllPath);
		xaudioLibrary = LoadLibraryA(xaudioLibPath);
		if (!xaudioLibrary)
		{
			char buffer[PATH_BUFSIZE];
			sprintf_s(buffer, "Can't load %s", xaudioLibPath);
			LogWindowsError(buffer, GetLastError());
			return false;
		}
	}
	return true;
...
const char* ts3plugin_name() {
	...
	if (!result) {
		...
		if (wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			return "Test Plugin";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
...
int ts3plugin_init() {
	...
	ts3Functions.getPluginPath(pluginPath, PATH_BUFSIZE, pluginId.c_str());

	printf("PLUGIN: App path: %s\nResources path: %s\nConfig path: %s\nPlugin path: %s\n", appPath, resourcesPath, configPath, pluginPath);
	if (!LoadXAudioLibrary(pluginPath, "lib/xaudio2_9redist.dll"))
	{
		return 1;
	}


	if (!Win32ExceptionBoundary([]() {
...
void ts3plugin_shutdown() {
	...
	pluginId.~basic_string();
	FreeLibrary(xaudioLibrary);
}
...
anyID getCurrentChannelClient(uint64 serverConnectionHandlerID, const char* name)
{
	anyID myID;
	uint64 myChannelID;
	...
	if (ts3Functions.getChannelClientList(serverConnectionHandlerID, myChannelID, &clientIds) != ERROR_ok) {
		ts3Functions.logMessage("Error querying channel clients", LogLevel_ERROR, "Plugin", serverConnectionHandlerID);
		return 0;
	}
	anyID result = 0;
	...
	{
		char buf[128];
		if (ts3Functions.getClientDisplayName(serverConnectionHandlerID, clientIds[i], buf, 128) != ERROR_ok)
		{
			ts3Functions.logMessage("Error getting client name", LogLevel_ERROR, "Plugin", serverConnectionHandlerID);
			break;
		}
		if (!strcmp(buf, name))
		{
...
int ts3plugin_processCommand(uint64 serverConnectionHandlerID, const char* input) {
	printf("PLUGIN: process command: '%s'\n", input);

	enum {
		CMD_NONE = 0, CMD_COMMAND,
		//CMD_JOIN,  CMD_SERVERINFO, CMD_CHANNELINFO, CMD_AVATAR, 
		//CMD_ENABLEMENU, CMD_SUBSCRIBE, CMD_UNSUBSCRIBE, CMD_SUBSCRIBEALL, CMD_UNSUBSCRIBEALL, CMD_BOOKMARKSLIST,
		CMD_UPDATE_3D, CMD_REVERB
	} cmd = CMD_NONE;

	std::basic_istringstream<char> inputStream{ std::string(input) };

	char s[32]{ 0 };
	inputStream.getline(s, 32, ' ');
	if (!strcmp(s, "update3d")) {
		cmd = CMD_UPDATE_3D;
	}
	if (!strcmp(s, "reverb")) {
		cmd = CMD_REVERB;
	...
	case CMD_UPDATE_3D:
		inputStream.getline(param1, sizeof(param1), ' '); //user name
		clientId = getCurrentChannelClient(serverConnectionHandlerID, param1);
		if (clientId)
		{
			return !Win32ExceptionBoundary([serverConnectionHandlerID, clientId, &inputStream]() {
				FLOAT32 x = 0;
				FLOAT32 y = 0;
				FLOAT32 z = 0;
				inputStream >> x >> y >> z;
				...
				serverClientVoiceMap[serverConnectionHandlerID].at(clientId).Update3D(&emitter);
				});
		}
		return 1;
		break;
	case CMD_REVERB:
		...
		if (clientId)
		{
			return !Win32ExceptionBoundary([serverConnectionHandlerID, clientId, &inputStream]() {
				...
				auto p = PRESET_PARAMS[index];
				serverClientVoiceMap[serverConnectionHandlerID].at(clientId).UpdateReverb(&p);
			});
		}
		return 1;
		break;
		...
		//	inputStream.getline(param1, sizeof(param1), ' ');
		//	inputStream.getline(param2, sizeof(param2), ' ');
		...
		//		ts3Functions.createReturnCode(pluginId.c_str(), returnCode, RETURNCODE_BUFSIZE);
	...
	case CMD_COMMAND:  /* /test command <command> */
		...
		if (param1) {
			...
			if (!pluginId.empty()) {
				...
				ts3Functions.sendPluginCommand(serverConnectionHandlerID, pluginId.c_str(), param1, PluginCommandTarget_CURRENT_CHANNEL, NULL, NULL);
			}
			else {
		...
		//		ts3Functions.setPluginMenuEnabled(pluginId.c_str(), menuID, enable);
		//	}
		//	else {
...
void ts3plugin_onConnectStatusChangeEvent(uint64 serverConnectionHandlerID, int newStatus, unsigned int errorNumber) {
	...
	{
		serverClientVoiceMap[serverConnectionHandlerID].clear();
	}
}
...
void ts3plugin_onClientMoveEvent(uint64 serverConnectionHandlerID, anyID clientID, uint64 oldChannelID, uint64 newChannelID, int visibility, const char* moveMessage) {
	...
	{
		if (myChannelID == oldChannelID) {
			serverClientVoiceMap[serverConnectionHandlerID].erase(clientID);
		}
...
void ts3plugin_onEditPlaybackVoiceDataEvent(uint64 serverConnectionHandlerID, anyID clientID, short* samples, int sampleCount, int channels) {
	Win32ExceptionBoundary([=]() {
		if (audioEngine)
		{
			auto& clientVoiceMap = serverClientVoiceMap[serverConnectionHandlerID];
			if (clientVoiceMap.find(clientID) == clientVoiceMap.end())
			{
				auto voice = audioEngine->CreateVoice();
				clientVoiceMap.insert({ clientID, std::move(voice) });
			}

			clientVoiceMap.at(clientID).SubmitBuffer(samples, sampleCount, channels);
		}
...
void ts3plugin_onEditPostProcessVoiceDataEvent(uint64 serverConnectionHandlerID, anyID clientID, short* samples, int sampleCount, int channels, const unsigned int* channelSpeakerArray, unsigned int* channelFillMask) {
	//char buffer[1024];
	//sprintf_s(buffer, "onEditPostProcessVoiceDataEvent client=%d sampleCount=%d channels=%d", clientID, sampleCount, channels);
